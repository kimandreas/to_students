...
def run_yolo(model,output_dir):
    ...
    cap = cv2.VideoCapture('/dev/video2') #set to USB Camera
    cap.set(3, 640)
    cap.set(4, 480)
        
    # classNames = ['Normal','Error']
    classNames = model.names if hasattr(model, 'names') else ['Object']
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if (classNames[cls]=='Truck'):
                    ...
                    cv2.putText(annotated_img, f"{classNames[cls]}: {confidence}", org, font, fontScale, color, thickness)
