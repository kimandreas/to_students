...
class DepthToMap(Node):
    ...
    def depth_callback(self, msg):
        try:
            ...
            with self.lock:
                h, w = depth.shape
                # ----- crop percentages (26 % width, 18 % height) -----
                crop_x = int(0.26 * w / 2) * 2
                crop_y = int(0.18 * h / 2) * 2

                depth_crop = depth[crop_y : h - crop_y, crop_x : w - crop_x]

                # Resize back to 640 × 480 using NEAREST to preserve integer depth
                self.depth_image = cv2.resize(depth_crop, (w, h), cv2.INTER_NEAREST)
