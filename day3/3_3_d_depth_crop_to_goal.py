...
class DepthClickNav(Node):
    def __init__(self):
        ...
        # Preview timer
        self.create_timer(0.2, self.compose_preview)
    ...
    def compose_preview(self):
        ...
        if click and self.K is not None:
            u, v = click
            ...
            if 0.2 < z < 5.0:
                ...
                try:
                    ...
                    #     [pt_map.point.x, pt_map.point.y], TurtleBot4Directions.NORTH)
                    ...
                    with self.lock:
                        self.clicked = None # reset click after sending goal
                        self.get_logger().info("click reset")
            ...
            for img in (rgb, depth_vis):
                cv2.drawMarker(img, (u, v), (0,255,0), cv2.MARKER_CROSS, 12, 2)
            cv2.putText(depth_vis, label, (u, v-6), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)

        self.display = np.hstack((rgb, depth_vis))

    # ---------------- GUI loop --------------------------------------
    def gui_loop(self):
        cv2.namedWindow("RGB | Depth", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("RGB | Depth", 1280, 480)
        cv2.setMouseCallback("RGB | Depth", self.on_mouse)
        while not self.gui_stop.is_set():
            if self.display is not None:
                cv2.imshow("RGB | Depth", self.display)
