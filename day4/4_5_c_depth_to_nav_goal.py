                    ...
                    if x < rgb_display.shape[1] and y < rgb_display.shape[0]:
                        ...
                        if z > 0:
                            ...
                            try:
                                ...
                                self.send_goal(pt_map.point.x, pt_map.point.y, pt_map.point.z)
                            except Exception as e:
...
if __name__ == '__main__':
    main()
# This code is a ROS2 node that subscribes to depth and RGB images from a TurtleBot4 robot, processes the images, and allows the user to click on a point in the RGB image to send a navigation goal to the robot. The node uses OpenCV for image processing and visualization, and TF2 for coordinate transformations.
# The node also handles docking and undocking of the robot, and provides feedback on the navigation status. The user can exit the program by pressing 'q'.
# The code is structured to handle exceptions and log important information for debugging and monitoring purposes.
